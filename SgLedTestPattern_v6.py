...
# DO I NEED THIS VALIDATION?
def half_tile_input_validation(prompt):
    while True:
        try:
...
# TODO: ask for user input of wall dimensions
...
wallPanelHeight = int_input_validation(
    '\n' + 'How many full-size tiles high do you need the pattern? '
)
...
W, H = (tileResWidth, tileResHeight)
...
while True:
    ...
    while indexNums[0] <= wallPanelWidth + 1:
        ...
        loop_counter1 += 1
        logging.debug('While 1 statement start ' + str(loop_counter1))
        ...
        if indexNums[1] == wallPanelHeight + 1:
            logging.debug('Start if indexNums A')
